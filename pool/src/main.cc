...
int readConfigFile( PoolInfo& cinfo, std::string filename ) {
    /* File open
     */
    std::ifstream ifs;
    ...
    while( std::getline( ifs, line ) ) {
        ...
        else if( key.compare("pool_type") == 0 ) {
            cinfo.pool_type = value;
        }
        else if( key.compare("global_pooling") == 0 ) {
            cinfo.global_pooling = value;
        }
        else {
            // invalid parameter !!
...
int main(int argc, char **argv) {
     ...
     */
    PoolInfo pinfo;
    if( readConfigFile( pinfo, configFileName ) != 0 ) {

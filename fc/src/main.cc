...
int readConfigFile( FcInfo& finfo, std::string filename ) {
    /* File open
     */
    std::ifstream ifs;
...
int main(int argc, char **argv) {
	...
	if( argc != 11 ) {
        ...
        std::cerr << "fc -i [IFM file] -w [Weight file] -b [bias file] -o [OFM file] -c [config file]" << std::endl;
		return -1;
	}
    ...
    /* construct FullyConnected test date information 
     */
    FcInfo finfo;
    if( readConfigFile( finfo, configFileName ) != 0 ) {
        std::cerr << "[Error] fail to process config file!" << std::endl;
        return 0;
    }


    /* Run FullyConnected operation test code 
     */
    test_kernel_fc( 
        (float*) obuf,
        (float*) ibuf,
